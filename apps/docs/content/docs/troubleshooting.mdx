...
**Symptom**: after assigning a task to a digital employee, the task status flips to `in_progress` immediately, but a long time passes with no sign of digital employee execution on the page; `multica daemon status` shows the daemon `online`.

**Likely causes** (ordered by frequency):

1. **Digital employee concurrency limit reached** — this digital employee's `max_concurrent_tasks` (default 6) is fully occupied by other running tasks
2. **Another task from the same digital employee is still running on the same task** — same digital employee × same task is forced to run sequentially (prevents duplicate execution)
3. **Digital employee has been archived** — after archival, new tasks still enqueue but can't be claimed, and they time out after 5 minutes (code-task G-01)
...
multica agent list                         # check digital employee archived state
...
- Same-task serialization → wait for the previous task to finish, or reassign to a different digital employee
- Digital employee archived → `multica agent restore <id>`
