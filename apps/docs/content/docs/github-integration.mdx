...
Connect a GitHub account or organization once in **Settings → GitHub**. After that, any pull request whose branch name, title, or body contains a task identifier (for example `MUL-123`) is **auto-linked** to that [task](/issues), appears under **Pull requests** in the task sidebar, and — when the PR is merged — moves the task to **Done**.

There is no per-task setup. The whole flow is identifier-driven.
...
| **Task sidebar → Pull requests** | Every PR auto-linked to this task, with title, repo, state (`Open` / `Draft` / `Merged` / `Closed`), and author. Click a row to jump to the PR on GitHub. |
...
| **Auto-status on merge** | When a PR transitions to `merged`, every linked task not already `Done` or `Cancelled` is moved to `Done`. The status change is timeline-logged with source `github_pr_merged`. |
...
- Workspace-scoped — only matches the workspace's own [task prefix](/workspaces). `FOO-1` in a workspace whose prefix is `MUL` is ignored, even if the integer matches another task.
- Deduplicated — listing `MUL-1, MUL-1` in the body links the task once.

You can reference **multiple tasks** in one PR. `Closes MUL-1, MUL-2` links the PR to both, and merging it advances both to `Done`.

## The auto-merge-to-Done rule

When a PR's `merged` field flips to `true`, every linked task is evaluated:

| Task current status | Result |
|---|---|
...
Closing a PR **without** merging it only updates the PR card's state to `Closed`. The linked tasks stay where they were — the user is the one who decides what closing-without-merge means.

<Callout type="info">
The action is attributed to the `system` actor on the timeline. Subscribers of the task receive an inbox notification for the status change, the same way they would if a human had moved it.
</Callout>
...
- **Manually linking a PR to a task** — there is no UI for this yet. If your team's convention puts identifiers in a place Multica isn't reading, add them to the PR title or body.
...
After disconnect, mirrored PR rows stay in the database so historical task sidebars still show what was linked, but no new webhook events from that installation will be accepted.

## Permissions and visibility

- **Connect / disconnect** require workspace **owner or admin**. Members see the card description but no Connect button.
- The **Pull requests** sidebar on a task is visible to anyone who can read the task — same permissions as the rest of task detail.
...
Three tables get created: `github_installation`, `github_pull_request`, `task_pull_request`. They cascade-delete with their workspace, so removing a workspace cleans them up automatically.
...
After that, open any PR whose branch / title / body contains a task identifier — within a few seconds the Pull requests block appears on that task's detail page.
...
- **Multi-PR-to-one-task is conservative on merge** — if two PRs both reference `MUL-123` and the first one merges, the task is moved to `Done` immediately. A follow-up change to wait for all linked PRs to resolve before advancing is in progress.

## Next

- [Tasks](/issues) — the task identifiers (`MUL-123`) referenced from PRs
- [Workspaces](/workspaces) — where the workspace-specific task prefix is set
