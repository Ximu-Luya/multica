...
- **Name** — display name
- **Digital employee** — who the run is dispatched to
- **Priority** — inherited by the `task` it produces (same semantics as task priority)
- **Description / prompt** — the work description the digital employee receives each run
...
- **Create task mode** (`create_issue`) — default, **recommended**. Each trigger first creates a task in the workspace (the title currently supports a single placeholder, `{{date}}`, which interpolates to the UTC date in `YYYY-MM-DD` format; any other `{{...}}` token is rejected at create-time so a typo cannot silently land as the literal string in your task titles), then assigns the task to the digital employee through the normal assignment flow. All work lands on the task board with the same history, comments, and status as a manually assigned task.
- **Run-only mode** (`run_only`) — skips task creation and enqueues a `task` directly. The run is invisible on the board — you can only see it in the autopilot's run history.
...
In **create task mode**, the inbound payload is appended to the new task's
description so the digital employee can read it inline. In **run-only mode**, the
payload is part of the run context the daemon hands the digital employee.
...
- `{"status":"skipped","run_id":"…","reason":"digital employee runtime is offline at dispatch time"}`
...
- Status (`task_created` / `running` / `completed` / `failed` / `skipped`)
- The linked task (create task mode) or `task` (run-only mode)
...
If an autopilot is important, design your own monitoring — for example, have the digital employee post a comment on success, and catch failures by noticing missing comments.
</Callout>
...
- [**Assign tasks to digital employees**](/assigning-issues) — a one-shot hand-off of a task to a digital employee
- [**@-mention digital employees in comments**](/mentioning-agents) — pull a digital employee in to take a look from a comment
