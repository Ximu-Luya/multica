...
The Multica CLI mirrors almost everything the Web UI can do (create [tasks](/issues), assign [digital employees](/agents), start the [daemon](/daemon-runtimes), and more). This page lists every top-level command with a one-line description. For the full set of flags and examples, run `multica <command> --help`.
