---
title: Tasks
description: The unit of work for every digital employee run, with a clear state machine, timeouts, and retry rules.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

A **task** is the unit of every [digital employee](/agents) run — [assigning a task to a digital employee](/assigning-issues), [@-mentioning a digital employee in a comment](/mentioning-agents), sending a message in [chat](/chat), or an [Autopilot](/autopilots) firing on schedule all produce a task. Multica puts it in a queue; a [daemon](/daemon-runtimes) picks it up and hands it off to the corresponding [AI coding tool](/providers), then writes the result back to the server when it finishes.

Tasks and [tasks](/issues) are two different objects. A single task can be assigned, @-mentioned, and manually rerun many times — each produces a **new** task.
...
2. **Only for task- and chat-triggered tasks** — Autopilot-triggered tasks do **not** retry automatically.
...
**How you'll know an Autopilot task failed**: a notification lands in your [Inbox](/inbox), and the associated task's status reverts from `in_progress` back to `todo`. The [Autopilots](/autopilots) page also shows the latest run result per autopilot.
</Callout>
...
- By default, targets the task's **current digital employee assignee** — useful when you want the rerun to follow the current assignment regardless of who ran the prior task.
- The execution-log retry button on a specific row sends that row's task ID alongside, so the rerun targets **the digital employee that ran that exact task** — not the current assignee. This makes per-row retry meaningful for digital department workers, parallel @-mention digital employees, or rows whose digital employee has since been displaced by a reassignment.
- **Cancels** the target digital employee's queued or running task on this task (if any). Tasks owned by other digital employees on the same task (e.g. parallel @-mention runs) are left alone.
- Creates a **brand-new** task — attempt count resets to 1, even if the original task hit the attempt ceiling.
- Starts a **fresh digital employee session** — the prior session ID is **not** inherited. A manual rerun means you've judged the previous output bad, so resuming the same conversation would replay the same poisoned state. (Automatic retry, by contrast, does inherit the session — that path is for infrastructure failures, not bad output.)
...
| Applicable sources | Tasks, chat | Tasks with a digital employee assignee |
| Digital employee picked | Same digital employee as the failed task | Source task's digital employee (UI per-row retry) or task's current assignee (CLI / no task_id) |
| Session inheritance | Yes (resumes prior session) | No (fresh session) |

## How a failed task affects task status

If a task-triggered task fails (and no automatic retry succeeds) because the task was assigned to a digital employee, **the task's status automatically rolls back from `in_progress` to `todo`** — so when you open the board you immediately see "this one needs another look." See [Tasks and projects](/issues).

## Can a task continue from the previous context

Yes — as long as the AI coding tool supports session resumption.

Multica pins the session ID **twice** during a task: once at the start (when the AI tool returns its first system message), and once at the end (on completion or failure). The first lets the daemon recover if it crashes mid-run; the second is reserved for the next **automatic retry**, where that ID is passed back so the digital employee can pick up the previous conversation and file state. **Manual rerun deliberately skips this** and starts a fresh session — see [Manual rerun vs. automatic retry](#manual-rerun-vs-automatic-retry).
...
- [Assigning tasks to digital employees](/assigning-issues) / [@-mentioning digital employees in comments](/mentioning-agents) / [Chat](/chat) / [Autopilots](/autopilots) — the four ways to trigger a task
