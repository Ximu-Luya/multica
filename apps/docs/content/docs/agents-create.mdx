---
title: Create and configure a digital employee
description: The minimum fields to create a digital employee, plus every optional setting — system instructions, environment variables, visibility, concurrency limit, and archiving.
---

import { Callout } from "fumadocs-ui/components/callout";

Creating a [digital employee](/agents) takes only two things: **a name** and **a choice of [AI coding tool](/providers)**. Everything else is optional — system instructions, model, environment variables, CLI arguments, visibility, concurrency limit — the defaults work fine. Get it running first and tune later; every field can be changed at any time.
...
Once that's in place, go to the **Digital employees** page in your workspace and click **+ New**, or use the CLI:
...
**System instructions** (`instructions`) are prepended to every task, telling the digital employee what role it plays and what rules to follow:

```text
You're a frontend code-review digital employee. When a task comes in, read the diff first. Focus only on:
- Styling issues (tailwind class names, box model)
- Accessibility (a11y)
Don't change code — leave suggestions in a comment.
```

When left blank (the default), the digital employee uses the native behavior of its underlying AI coding tool with no extra constraints.
...
- **Private** (`private`) — only workspace owners, admins, or the digital employee's creator can assign it

New digital employees default to `private`.

**Private does not mean hidden** — every member sees a private digital employee's name and description in the list, they just can't read sensitive config (env values never appear in digital employee list/get responses; MCP config is masked for non-owners). Full meaning in [Digital employees → Who can assign a digital employee](/agents#who-can-assign-an-agent).

## Concurrency limit

**Concurrency limit** (`max_concurrent_tasks`) controls how many tasks this digital employee can run in parallel at once. The default is **6**. New tasks that hit the cap queue up — they aren't rejected.

This is only the "digital employee layer" of a two-tier limit — the daemon itself enforces a broader cap (default 20), and whichever is tighter wins. Details in [Daemon and runtimes → How many tasks can run in parallel](/daemon-runtimes#how-many-tasks-can-run-in-parallel).
...
A created digital employee can have **Skills** attached — **knowledge packs** (`SKILL.md` + supporting files) automatically delivered to the AI coding tool at task execution time. You can create a new skill, import from GitHub or ClawHub, or scan one from an existing skill directory on your machine. See [Skills](/skills).

## Archive and restore

Digital employees you no longer use can be **archived** — they disappear from everyday views, but their historical data (tasks run, comments posted) is fully preserved. **Restore** them anytime to put them back to work.

<Callout type="warning">
**Archiving immediately cancels every unfinished task belonging to the digital employee** — running, dispatched, and queued tasks are all marked `cancelled` and won't continue. If you have an important task in flight, let it finish before archiving.
</Callout>

Archived digital employees can't be assigned new tasks.
...
- [Assigning tasks to digital employees](/assigning-issues) — put your new digital employee to work
