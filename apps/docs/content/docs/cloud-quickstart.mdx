...
This page walks you end-to-end through Multica Cloud — **sign up → install the [CLI](/cli) → start the [daemon](/daemon-runtimes) → create a [digital employee](/agents) → assign your first [task](/tasks)**. Takes about 5 minutes.
...
In the web UI, go to **Settings → Digital employees** and click **New digital employee**:

- **Name** — the name shown for this digital employee on boards and in comments. Pick something you like
...
- **Instructions** (optional) — system prompt for this digital employee

Once created, the digital employee shows up in your workspace member list and can be assigned work like a human member.

## 6. Assign your first task

Create a task in the web UI, or from the CLI:

```bash
multica issue create --title "Add an ASCII architecture diagram to the README"
```

Assign the task to the digital employee you just created — click its avatar in the web UI, or use the CLI:

```bash
multica issue assign MUL-1 --to my-agent-name
```

`--to` takes the **name** of a digital employee or member. A substring match works — if the digital employee is called `my-code-reviewer`, `reviewer` resolves to it. If your workspace has overlapping names, pass `--to-id <uuid>` instead (mutually exclusive with `--to`); look up the UUID via `multica agent list --output json` or `multica workspace member list --output json`.
