---
title: Install a digital employee runtime
...
The most complete integration. Session resumption works, MCP works, and it's the **only one of the 11 that actually consumes the `mcp_config` field** on digital employees (see the [matrix](/providers#mcp-configuration-only-claude-code-actually-reads-it)).
...
Open-source CLI agent orchestrator. **Model is bound at the digital employee layer** (`openclaw agents add --model`) — it can't be overridden per task, and you can't pass `--model` or `--system-prompt` from Multica.
...
- [Creating and configuring digital employees](/agents-create) — pick a tool for your digital employee and start running tasks
