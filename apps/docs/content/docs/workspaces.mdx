...
A workspace is **the self-contained space where a group collaborates in Multica** — every [task](/issues), [member](/members-roles), [comment](/comments), and [digital employee](/agents) belongs to one. The task list, member roster, and digital employee configuration you see after logging in are all scoped to the current workspace; **switching workspaces replaces the entire view**.
...
- **Task prefix** — the prefix for every task number in the workspace (the `MUL` in `MUL-123`). Uppercase letters and digits, up to 10 characters.

<Callout type="warning">
**Avoid changing the task prefix.** Task numbers are rendered with the current prefix — change it and `MUL-5` instantly becomes `NEW-5`. Every external link, Slack mention, and historical reference in comments breaks against the old number. Treat the task prefix as "set at creation, never touched."
</Callout>
...
Every task created in a workspace is automatically assigned a number in the format `<prefix>-<digits>` — `MUL-1`, `MUL-2`, `MUL-3`. A few properties:

- **Sequential and unique within a workspace** — each workspace keeps its own counter; workspaces don't interfere with each other.
- **Not manually assignable** — when you create a task you only supply a title; the number is assigned by the system.
- **Never reclaimed on delete** — delete `MUL-5` and the next new task is `MUL-6`, not `MUL-5`.
...
- Every task, project, comment, and reaction
- Every attachment
- Every membership and pending invitation
- Every digital employee configuration along with its task history
...
- [Tasks and projects](/issues) — the core work objects inside a workspace
