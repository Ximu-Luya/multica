...
For guidance on picking a tool when creating a digital employee, see [Creating and configuring digital employees](/agents-create).
...
**For your decision**: if your workflow needs digital employees to preserve context across tasks (failure retries, manual reruns, conversational iteration), pick only from the ✅ row.
...
If you set `mcp_config` in a digital employee configuration but pick a tool other than Claude Code, your MCP servers have **no effect** on that digital employee. MCP integration currently covers Claude Code only.
</Callout>
...
- [Creating and configuring digital employees](/agents-create) — pick a tool for your digital employee
